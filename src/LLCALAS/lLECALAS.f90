...
!    act(NC),actas(NC),NGRUPASD(NMG),NPUNTASD(NMG)
   ...
   !   DATA MAINSG/4*1,4*2,2*3,3*4,5,6,7,8,9,2*10,11,12,2*13,2*14,4*15,3*16,3*17,2*18,19,20,2*21,22,3*23,24,25,26,3*27,28,29,30,31,32/     
     ...
     874.3,629.,4*1.,475.8,0.,-465.2,1.,794.4,1.,-241.7,1.,-906.5,5*1. /
      ...
      DATA A22/902.2,1.,1.64,689.6,-348.2,1.,1.,-109.8,-851.6,1010.,2*1.,942.2,&
      4*1.,-75.5,1302.,2*1.,0.,1.,175.8,164.4,1.,-944.9,5*1./    
      DATA A23/556.7,425.7,-1.77,3629.,-30.7,150.8,150.8,1538.6,1.,400.,2*1.,&
      446.3,1.,95.18,3*1.,490.9,-154.5,2*1.,0.,1.,481.3,7*1.      /
      DATA A24/575.7,1.,-11.19,-175.6,-159.,2*1.,32.92,-16.13,-328.6,8*1.,&
      534.7,2*1.,179.9,1.,0.,-246.,7*1.           /                   
      DATA A25/527.5,1.,358.9,337.7,536.6,2*1.,-269.2,-538.6,211.6,1.,-278.2,&
      572.7,343.1,5*1.,124.8,1.,125.3,139.8,963.,0.,7*1.    /      
      DATA A26/269.2,1.,363.5,1023.,53.37,20*1.,0.,6*1.          /      
      DATA A27/-300.,1.,-578.2,-390.7,183.3,2*1.,-873.6,-637.3,2*1.,&
      -208.4,5*1.,18.98,1.,-387.7,134.3,924.5,4*1.,0.,5*1.     /            
...
!ccccccccccccccccc Escritura de los par�metros de asociaci�n ALFONSINAccccccccccccccccc
 ...
 218	FORMAT(/,X,'"ASSOC GROUP COMPOSITION" ',/,23X,'COMPONENTES',/,' GRUPO 	  #  SIT ASOC  ',5I5,/) 

	DO ja=1,NGA
	WRITE(1,219) ja,MASS(ja)  , (rngoh(i,ja),i=1,nc) 
 219	FORMAT(3X,I3,9X,I3,6X,9f5.1)
	END DO
